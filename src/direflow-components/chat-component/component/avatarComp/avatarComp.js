    ...
    return (
        <Styled styles={styles}>
            <div className="avatar-comp">
                {avatarUrl ? (
                    <img className="avatar-comp_img" src={avatarUrl} />
                ) : (
                    <div className="default" style={{ backgroundImage: "url(" + morePagePersonIcon + ")" }}></div>
                )}
            </div>
        </Styled>
    );
    ...
    useEffect(() => {
        ...
        urls.map((item) => {
            if (/(http|https):\/\/([\w.]+\/?)\S*/.test(item)) {
                resultArr.push(item);
            } else if (item && item.startsWith("data:image")) {
                resultArr.push(item);
            } else {
            ...
            <div className="avatar-mult-comp">
                ...
                {listArr.length == 3 && (
                    <div className="avatar-mult-comp-3">
                        <div
                            className="avatar-mult-comp-3_top"
                            style={{ backgroundImage: "url(" + morePagePersonIcon + ")" }}
                        >
                ...
                {listArr.length > 3 && listArr.length < 9 && (
                    <div className="avatar-mult-comp-4">
                        <div className="avatar-mult-comp-4_1">
                            <div
                                className="avatar-mult-comp-4_1_imgBox"
                                style={{ backgroundImage: "url(" + morePagePersonIcon + ")" }}
                            >
                                {listArr[0] && <img src={listArr[0]} />}
                            </div>
                            <div
                                className="avatar-mult-comp-4_1_imgBox"
                                style={{ backgroundImage: "url(" + morePagePersonIcon + ")", marginLeft: "1px" }}
                            >
                                {listArr[1] && <img src={listArr[1]} />}
                            </div>
                        </div>
